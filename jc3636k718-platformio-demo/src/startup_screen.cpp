...
bool startup_screen_begin()
{
    ...
    lv_obj_t *mark =
        make_label(badge, "JC", &lv_font_montserrat_32, cyan);
    lv_obj_align(mark, LV_ALIGN_CENTER, 0, -24);

    lv_obj_t *model =
        make_label(badge, "3636 / K718", &lv_font_montserrat_16, text);
    lv_obj_align(model, LV_ALIGN_CENTER, 0, 16);

    lv_obj_t *caption =
        make_label(badge, "LVGL LAB", &lv_font_montserrat_14, muted);
