...
public class DeviceController {
    ...
    /** Full latest reply kept by Bridge, outside the compact device snapshot. */
    public record DeviceReply(
            @JsonProperty("thread_id") String threadId,
            String title,
            String state,
            String text,
    ...
    @GetMapping("/device/sessions/{threadId}/reply")
    public ResponseEntity<DeviceReply> fullReply(
            @PathVariable String threadId) {
        Optional<CodexStateStore.SessionSnapshot> session =
                stateStore.findSession(threadId);
        if (session.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        CodexStateStore.SessionSnapshot value = session.get();
        String text = firstNonBlank(value.lastMessage(), value.preview(), "");
        return ResponseEntity.ok(new DeviceReply(
                value.id(),
                firstNonBlank(value.name(), value.preview(), "Codex 会话"),
                value.status(),
                text,
                text.length()));
    }
    ...
    private DeviceSession toDeviceSession(
        ...
        return new DeviceSession(
                ...
                devicePreview(session.lastMessage(), session.preview()),
                deviceState(session),
                session.totalTokens(),
                session.lastTokens(),
    ...
    private static String devicePreview(String message, String fallback) {
        String value = firstNonBlank(message, fallback, "");
        if (value.length() <= 480) {
            return value;
        }
