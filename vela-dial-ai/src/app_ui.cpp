...
lv_obj_t *s_input_dots = nullptr;
// LVGL keeps the long-label animation template by pointer. Keep this
// template static so the repeat pause remains valid after render_detail()
// returns. The label then scrolls in one direction, pauses at the end, and
// starts again from the beginning instead of playing back like a wobble.
lv_anim_t s_reply_marquee_template = {};
bool s_reply_marquee_template_ready = false;
...
bool s_touch_navigation_consumed = false;
bool s_screen_touch_event_registered = false;
Surface s_touch_start_surface = Surface::Quota;
lv_point_t s_touch_start = {};
lv_point_t s_touch_last = {};
...
const BridgeSession *bridge_session_for_snapshot(
    const BridgeSnapshot &snapshot,
    const char *thread_id);
bool detail_session_changed(
    const BridgeSession *before,
    const BridgeSession *after);
void cancel_dwell();
void show_error(
    const char *title,
    const char *subtitle,
...
void input_dots_anim_cb(void *var, int32_t value);
...
void copy_text(char *destination, size_t capacity, const char *source)
{
    ...
    // The offline font always contains straight ASCII quotes. Normalize
    // typographic quotes from Codex so dynamic replies remain readable even
    // before the commercial on-demand glyph cache is added.
    char *read = destination;
    char *write = destination;
    while (*read != '\0') {
        const uint8_t first = static_cast<uint8_t>(read[0]);
        if (first == 0xE2U &&
            read[1] != '\0' &&
            read[2] != '\0') {
            const uint8_t second =
                static_cast<uint8_t>(read[1]);
            const uint8_t third =
                static_cast<uint8_t>(read[2]);
            if (second == 0x80U &&
                (third == 0x9CU || third == 0x9DU)) {
                *write++ = '"';
                read += 3;
                continue;
            }
            if (second == 0x80U &&
                (third == 0x98U || third == 0x99U)) {
                *write++ = '\'';
                read += 3;
                continue;
            }
            // LVGL's compact font intentionally keeps the ASCII punctuation
            // set. Normalize Unicode dash variants from Codex/server text so
            // Wi-Fi and prose never fall back to a missing-glyph box.
            if ((second == 0x80U && third >= 0x90U && third <= 0x95U) ||
                (second == 0x88U && third == 0x92U)) {
                *write++ = '-';
                read += 3;
                continue;
            }
...
void format_usage_date(const char *iso_date, char *output, size_t capacity)
{
    ...
    if (iso_date == nullptr || strlen(iso_date) < 10) {
        copy_text(output, capacity, "最近一天");
        return;
    }
    const int month =
    ...
    snprintf(output, capacity, "%d月%d日", month, day);
}
...
void input_dots_anim_cb(void *var, int32_t value)
{
    ...
    const int frame = clamp_int(value, 0, 3);
    lv_label_set_text(label, frames[frame]);
}
...
void render_header(const char *page_name)
{
    ...
    const bool bridge_online = s_bridge_snapshot.bridge_online;
    const bool ap_active = connectivity.access_point_active;

    // Keep connection state visible on every screen. A full-screen spinner is
    // reserved for first-time provisioning; once configured, the product
    // remains usable while Wi-Fi/Bridge reconnect in the background.
    const char *wifi_label = ap_active && !wifi_online
        ? "AP"
        : (wifi_online ? "WiFi" : "WiFi...");
    const char *bridge_label = bridge_online ? "Bridge" : "Bridge...";
    const lv_color_t wifi_color = ap_active && !wifi_online
        ? kAmber
        : (wifi_online ? kGreen : kRed);
    const lv_color_t bridge_color = bridge_online ? kGreen : kAmber;

    make_dot(s_root, 24, 20, 5, wifi_color);
    lv_obj_t *wifi = make_label(
        s_root, wifi_label, &lv_font_montserrat_14, kMuted);
    lv_obj_set_pos(wifi, 33, 15);
    lv_obj_set_width(wifi, 47);
    lv_label_set_long_mode(wifi, LV_LABEL_LONG_CLIP);

    lv_obj_t *bridge_dot = make_dot(s_root, 294, 20, 5, bridge_color);
    (void)bridge_dot;
    lv_obj_t *bridge = make_label(
        s_root, bridge_label, &lv_font_montserrat_14, kMuted);
    lv_obj_set_pos(bridge, 303, 15);
    lv_obj_set_width(bridge, 48);
    lv_label_set_long_mode(bridge, LV_LABEL_LONG_CLIP);
...
Surface required_network_surface()
{
    const ConnectivitySnapshot &connectivity =
        s_bridge_snapshot.connectivity;
    // A configured device should land on the AI surface immediately. Wi-Fi
    // and Bridge continue in the background; showing CONNECTING or BRIDGE
    // OFFLINE here created a second loading page before the real UI.
    ...
    const char *thread_id)
{
    ...
    for (uint8_t index = 0;
         index < snapshot.session_count &&
         index < VELA_MAX_BRIDGE_SESSIONS;
         ++index) {
        const BridgeSession &session = snapshot.sessions[index];
        if (same_text(session.thread_id, thread_id)) {
            return &session;
        }
    }
    return nullptr;
    ...
    const BridgeSession *after)
{
    ...
    const bool state_changed = before->state != after->state;
    const bool metadata_changed =
           before->needs_feedback != after->needs_feedback ||
           before->approval.present != after->approval.present ||
           !same_text(
               before->approval.approval_id,
               after->approval.approval_id) ||
           !same_text(before->title, after->title);
    // Streaming deltas may update the server's preview while the turn is
    // still running. Keep the spinner/dots alive during that phase; redraw
    // once the state becomes complete so the final full reply appears.
    const bool reply_changed =
        !same_text(before->summary, after->summary);
    const bool stable_reply_changed =
        after->state != BridgeSessionState::Running && reply_changed;
    return state_changed || metadata_changed || stable_reply_changed;
}
...
void render_bridge_offline()
{
    ...
    make_centered_label(
        s_root,
        wifi_online ? "BRIDGE OFFLINE" : "WIFI OFFLINE",
...
void render_usage_switcher()
{
    ...
    lv_obj_t *switcher =
        make_panel(s_root, x, 226, width, 30, kPanel, kBorder, 1, 15);
    ...
    for (int index = 0; index < pages; ++index) {
        ...
        lv_obj_set_pos(item, 6 + index * item_width, 5);
        if (index == s_quota_index) {
            make_dot(
                switcher,
                6 + index * item_width + (item_width - 4) / 2,
                25,
                4,
                kCyan);
        }
...
{
    const bool token_page = usage_page_is_tokens();
    if (!token_page) {
        char token_text[48] = "-- TOKEN / 最近一天";
...
void render_quota()
{
    if (usage_page_is_tokens()) {
        ...
        format_usage_date(
            ...
            sizeof(date_text));
        char caption[40];
        snprintf(caption, sizeof(caption), "%s / 最近一天", date_text);
        make_centered_label(
            s_root,
            caption,
            &lv_font_montserrat_14,
        ...
        snprintf(
            ...
            sizeof(value_text),
            "%u",
            static_cast<unsigned>(quota.remaining_percent));
        lv_obj_t *number =
    ...
    bool selected)
{
    ...
    lv_obj_set_style_anim_speed(title, 22, LV_PART_MAIN);
...
void render_detail()
{
    ...
    const bool session_running =
        s_selected_session->status == SessionStatus::Running;
    ...
    lv_obj_t *core =
        make_panel(
            ...
            54,
            kPanel,
            session_running ? kBlue : kCyan,
            1,
            27);
    lv_obj_t *core_label =
        make_label(core, "AI", &lv_font_montserrat_16, kText);
    lv_obj_center(core_label);
    if (session_running) {
        lv_obj_t *spinner = lv_spinner_create(s_root, 900, 76);
        lv_obj_set_pos(spinner, 143, 95);
        lv_obj_set_size(spinner, 74, 74);
        lv_obj_set_style_arc_width(spinner, 1, LV_PART_MAIN);
        lv_obj_set_style_arc_color(spinner, kBorder, LV_PART_MAIN);
        lv_obj_set_style_arc_width(spinner, 4, LV_PART_INDICATOR);
        lv_obj_set_style_arc_color(spinner, kBlue, LV_PART_INDICATOR);
        lv_obj_remove_style(spinner, nullptr, LV_PART_KNOB);
        lv_obj_clear_flag(spinner, LV_OBJ_FLAG_CLICKABLE);
    } else {
        lv_obj_t *orbit = lv_arc_create(s_root);
        lv_obj_set_pos(orbit, 143, 95);
        lv_obj_set_size(orbit, 74, 74);
    ...
    lv_obj_t *state = make_panel(
        s_root, 103, 172, 154, 26, kBg, kBg, 0, 13);
    make_dot(state, 13, 9, 7, accent);
    make_centered_label(
        state,
        session_running ? "正在输入" : status_text(s_selected_session->status),
        kCjk,
        accent,
        1,
        120);
    if (session_running) {
        s_input_dots = make_label(
            state, "", &lv_font_montserrat_20, kBlue);
        lv_obj_set_pos(s_input_dots, 112, 1);
        lv_obj_set_width(s_input_dots, 28);
        lv_obj_set_height(s_input_dots, 24);
        lv_anim_t dots_anim;
        lv_anim_init(&dots_anim);
        lv_anim_set_var(&dots_anim, s_input_dots);
        lv_anim_set_exec_cb(&dots_anim, input_dots_anim_cb);
        lv_anim_set_values(&dots_anim, 0, 3);
        lv_anim_set_time(&dots_anim, 1200);
        lv_anim_set_repeat_count(&dots_anim, LV_ANIM_REPEAT_INFINITE);
        lv_anim_set_path_cb(&dots_anim, lv_anim_path_step);
    ...
    make_centered_label(
        ...
        207,
        180);
    lv_obj_t *reply_panel = make_panel(
        s_root, 22, 226, 316, 76, kPanel, kBorder, 1, 14);
    lv_obj_t *summary_text = make_label(
        reply_panel,
        session_running ? "正在生成回复" : s_selected_session->summary,
        &lv_font_montserrat_16,
        kText);
    lv_obj_set_pos(summary_text, 11, 7);
    lv_obj_set_width(summary_text, 294);
    // Use a larger wrapped viewport so the device receives and reveals the
    // later part of a reply instead of showing only the first line. LVGL's
    // circular mode scrolls vertically when the wrapped text exceeds the
    // panel height, then pauses and restarts from the beginning.
    lv_obj_set_height(summary_text, 60);
    lv_obj_set_style_text_align(
        summary_text, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN);
    if (!s_reply_marquee_template_ready) {
        lv_anim_init(&s_reply_marquee_template);
        s_reply_marquee_template.act_time = -450;
        s_reply_marquee_template.repeat_delay = 900;
        s_reply_marquee_template_ready = true;
    }
    lv_obj_set_style_anim(
        summary_text,
        &s_reply_marquee_template,
        LV_PART_MAIN);
    lv_obj_set_style_anim_speed(summary_text, 24, LV_PART_MAIN);
    lv_label_set_long_mode(summary_text, LV_LABEL_LONG_SCROLL_CIRCULAR);
    ...
    if (s_selected_session->pending_approval ||
        ...
        lv_obj_set_pos(chevron, 300, 311);
        if (acquiring) {
            s_dwell_bar = make_progress_bar(
                s_root, 105, 340, 150, 3, action_color);
        }
    }
}
...
void render_surface()
{
    ...
    if (s_input_dots != nullptr) {
        lv_anim_del(s_input_dots, input_dots_anim_cb);
        s_input_dots = nullptr;
    }
    lv_obj_clean(s_root);
    s_dwell_bar = nullptr;
...
void handle_quota_detent(int direction)
{
    ...
    if (direction > 0) {
        if (static_cast<int>(s_quota_index) + 1 >= pages) {
            open_sessions(false, true);
            return;
        }
        ++s_quota_index;
    } else if (s_quota_index > 0) {
        --s_quota_index;
    }
    render_surface();
}
...
void sync_bridge_snapshot(bool force_render)
{
    ...
    // `generation` is a local publication counter and advances for every
    // HTTP poll, even when the payload is byte-for-byte unchanged. It must
    // not trigger a full LVGL tree rebuild: doing so restarts animations,
    // flashes the page, and makes the reply marquee appear to wobble.
    ...
    // While a detail page is open, changes to another session (or token
    // counters) should not tear down the active spinner/marquee. Rebuild the
    // backing session data, but redraw the detail page only when its visible
    // title/status/reply actually changed.
    const bool detail_changed =
        detail_session_changed(
            bridge_session_for_snapshot(
                s_bridge_snapshot,
                s_selected_thread_id),
            bridge_session_for_snapshot(
                latest,
    ...
    const bool data_changed =
        s_surface == Surface::Detail && s_selected_thread_id[0] != '\0'
            ? (detail_changed ||
               latest.connectivity.revision != s_connectivity_revision ||
               latest.bridge_online != s_bridge_snapshot.bridge_online ||
               latest.operation.updated_ms !=
    ...
    if (snapshot_changed || force_render) {
        rebuild_sessions_from_bridge();
    } else {
        s_selected_session =
            find_session(s_selected_thread_id);
...
bool handle_direct_swipe(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        s_touch_navigation_consumed = false;
        s_touch_start_surface = s_surface;
        s_touch_start = point;
        s_touch_last = point;
        return false;
    }
    ...
    if (s_touch_navigation_consumed) {
        return false;
    }

    const int dx = s_touch_last.x - s_touch_start.x;
    const int dy = s_touch_last.y - s_touch_start.y;
    const int horizontal = abs(dx);
    const int vertical = abs(dy);
    if (horizontal < 36 || horizontal < vertical + 10) {
        return false;
    }

    Serial.printf(
        "[UI] Direct swipe surface=%u dx=%d dy=%d\n",
        static_cast<unsigned>(s_touch_start_surface),
        dx,
        dy);

    // The CST816 coordinate direction varies with touch-panel mounting.
    // Quota has only one horizontal destination, so either raw direction
    // means the user's forward swipe and avoids calibration-specific failure.
    if (s_touch_start_surface == Surface::Quota) {
        s_touch_navigation_consumed = true;
        open_sessions(false, true);
        return true;
    }
...
void screen_touch_event(lv_event_t *event)
{
    (void)handle_direct_swipe(event);
}
...
void app_ui_begin()
{
    ...
    if (!s_screen_touch_event_registered) {
        lv_obj_add_event_cb(
            screen, screen_touch_event, LV_EVENT_ALL, nullptr);
        s_screen_touch_event_registered = true;
    }
